...
for beta in betas:
    bids_1p, exp_1p_in, cvar_1p = solve_one_price_cvar(beta, scen_in, prob_in, DA_in, Bal_in, Wind_in)
    exp_1p_oos = eval_oos_one_price(bids_1p)

    bids_2p, exp_2p_in, cvar_2p = solve_two_price_cvar(beta, scen_in, prob_in, DA_in, Bal_in, Wind_in)
    ...
    print(f"{beta:>6.2f} | {exp_1p_in:>13,.2f} | {exp_1p_oos:>13,.2f} | {cvar_1p:>12,.2f} | {exp_2p_in:>13,.2f} | {exp_2p_oos:>13,.2f} | {cvar_2p:>12,.2f}")
...
for ax, results, label, color, neutral_profit in [
    ...
    ax.set_title(f"Task 1.4 – {label} Scheme\nExpected Profit vs CVaR$_{{α={ALPHA}}}$ (200 in-sample)")
...
for ax, results, label, c_in, c_oos in [
    ...
    ax.set_title(f"Task 1.4 – {label} Scheme\nIn-sample vs Out-of-sample E[Profit] across β")
    ax.set_xlabel("β (risk-aversion weight)")
    ax.set_ylabel("Expected Profit (€)")
