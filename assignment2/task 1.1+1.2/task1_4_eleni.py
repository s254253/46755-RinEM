...
n_in_sample = 200
unique_scenarios = df_full['Scenario_ID'].unique()
np.random.seed(42) # For consistent results
selected_ids = np.random.choice(unique_scenarios, n_in_sample, replace=False)

# 3. Filter the dataframe
df = df_full[df_full['Scenario_ID'].isin(selected_ids)].copy()

# 4. RE-NORMALIZE: Crucial for correct Expected Value and CVaR scaling
current_prob_sum = df[['Scenario_ID', 'Probability']].drop_duplicates()['Probability'].sum()
df['Probability'] = df['Probability'] / current_prob_sum
...
def solve_one_price_cvar(beta):
    ...
    # new
    # m.setObjective(exp_profit + beta * (zeta - cvar), GRB.MAXIMIZE)
    #m.setObjective(exp_profit + beta * (cvar), GRB.MAXIMIZE)
    # old
    ...
    print(f"\n--- beta = {beta} ---")
    print("eta:", eta.X)
    print("First 5 bids:", [round(P_DA[t].X, 2) for t in list(hours)[:5]])
    print("Max lambda:", max(lambda_w[w].X for w in scenarios))
    print("--------------------------------")
