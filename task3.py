...
lines = pd.read_csv(
    'transmission.txt',
    header=None,
    names=['from_node', 'to_node', 'susceptance', 'capacity_MW']
)
...
for _, row in lines.iterrows():
    ...
    model.addConstr(
        flow[l] == b * (theta[i] - theta[j]),
        name=f'dc_flow_{l}'
    )

    model.addConstr(flow[l] <= row.capacity_MW, name=f'line_max_{l}')
    model.addConstr(flow[l] >= -row.capacity_MW, name=f'line_min_{l}')
...
for n in buses:
    model.addConstr(
        gp.quicksum(
            ...
            if generators.loc[generators.id == g, 'bus'].values[0] == n
        )
        + gp.quicksum(
            pw[w] for w in wind_farms.id
            if wind_farms.loc[wind_farms.id == w, 'bus'].values[0] == n
        )
        - gp.quicksum(
            pd[d] for d in demands.id
            if demands.loc[demands.id == d, 'bus'].values[0] == n
        )
        + gp.quicksum(
...
for g in generators.id:
    bus = generators.loc[generators.id == g, 'bus'].values[0]
    output = pg[g].X
...
for w in wind_farms.id:
    bus = wind_farms.loc[wind_farms.id == w, 'bus'].values[0]
    output = pw[w].X
...
print('\nDemands:')
for d in demands.id:
    bus = demands.loc[demands.id == d, 'bus'].values[0]
    served = pd[d].X
